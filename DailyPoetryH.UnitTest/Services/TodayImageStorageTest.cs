...
public class TodayImageStorageTest : IDisposable
{
    ...
    private static void RemoveImageFile() => File.Delete(TodayImageStorage.TodayImagePath);

    [Fact]
    public async Task GetTodayImageAsync_Default()
    {
        var preferenceStorageMock = new Mock<IPreferenceStorage>();
        preferenceStorageMock.Setup(p => p.Get(TodayImageStorage.FullStartDateKey, TodayImageStorage.FullStartDateDefault))
            .Returns(TodayImageStorage.FullStartDateDefault);
        preferenceStorageMock.Setup(p => p.Get(TodayImageStorage.ExpiresAtKey, TodayImageStorage.ExpiresAtDefault))
            .Returns(TodayImageStorage.ExpiresAtDefault);
        preferenceStorageMock.Setup(p => p.Get(TodayImageStorage.CopyrightKey, TodayImageStorage.CopyrightDefault))
            .Returns(TodayImageStorage.CopyrightDefault);
        preferenceStorageMock.Setup(p => p.Get(TodayImageStorage.CopyrightLinkKey, TodayImageStorage.CopyrightLinkDefault))
        ...
        Assert.Equal(TodayImageStorage.FullStartDateDefault, todayImage.FullStartDate);
        ...
        Assert.Equal(TodayImageStorage.CopyrightLinkDefault, todayImage.CopyrightLink);
        ...
        var todayImageHash = BitConverter.ToString(md5.ComputeHash(todayImage.ImageBytes));

        Assert.Equal(resourceImageHash, todayImageHash);

        File.Delete(TodayImageStorage.TodayImagePath);

        var bytesToWrite = Encoding.ASCII.GetBytes(DateTime.Now.Ticks.ToString());
        await using (var todayImageStream = new FileStream(TodayImageStorage.TodayImagePath, FileMode.Create))
            await using (var todayImageWriter = new BinaryWriter(todayImageStream))
        {
    ...
    public async Task SaveTodayImageAsync_Default()
    {
        ...
        preferenceStorageMock.Verify(
            p => p.Set(TodayImageStorage.ExpiresAtKey, todayImageToUpdate.ExpiresAt), Times.Once);
        preferenceStorageMock.Verify(
            p => p.Set(TodayImageStorage.FullStartDateKey, todayImageToUpdate.FullStartDate), Times.Once);
        preferenceStorageMock.Verify(
            p => p.Set(TodayImageStorage.CopyrightKey, todayImageToUpdate.Copyright), Times.Once);
        preferenceStorageMock.Verify(
            p => p.Set(TodayImageStorage.CopyrightLinkKey, todayImageToUpdate.CopyrightLink), Times.Once);
        Assert.Equal(todayImageToUpdate.ImageBytes.Length, new FileInfo(TodayImageStorage.TodayImagePath).Length);

        todayImageToUpdate.ExpiresAt = DateTime.Now;
        var todayImageFileLastWriteTime = new FileInfo(TodayImageStorage.TodayImagePath).LastWriteTime;
        ...
        preferenceStorageMock.Verify(
            p => p.Set(TodayImageStorage.CopyrightLinkKey, todayImageToUpdate.CopyrightLink), Times.Once);
        Assert.Equal(todayImageFileLastWriteTime, new FileInfo(TodayImageStorage.TodayImagePath).LastWriteTime);
    }
}
