...
public class BingImageServiceTest
{
    ...
    public async Task CheckUpdateAsync_TodayImageExpired()
    {
        ...
        todayImageStorageMock.Verify(p => p.GetTodayImageAsync(false), Times.Once);
        todayImageStorageMock.Verify(
            p => p.SaveTodayImageAsync(checkUpdateResult.todayImage, false), Times.Once);
        alertServiceMock.Verify(
            p => p.AlertAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()), Times.Never);

        Assert.Equal((byte)0xFF, checkUpdateResult.todayImage.ImageBytes[0]);
        Assert.Equal((byte)0xD8, checkUpdateResult.todayImage.ImageBytes[1]);
        Assert.Equal((byte)0xFF, checkUpdateResult.todayImage.ImageBytes[2]);
    }
}
